...
class Ipv4Options(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'ipv4_options')
        ...
        inputdict['ipv4_option'] = config.get_field_value('ipv4_option')
        inputdict['option'] = config.get_field_value('option')
        # END OF FETCHING THE LEAF PARAMETERS
        dev_pro_name = inputdict['name']
        if util.isNotEmpty(inputdict['ipv4_option']):
                if util.isNotEmpty(inputdict['option']):
        		dev_pro_name = "collect_ipv4_%s_%s" % (inputdict['ipv4_option'],inputdict['option'])
                else:
        		dev_pro_name = "collect_ipv4_%s" % (inputdict['ipv4_option'])
        ...
        if util.isNotEmpty(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name')):
          ...
          netflow_flow_records_flow_record_collect_ipv4_object = devices.device.netflow.flow_records.flow_record.collect.ipv4.ipv4()
          if util.isNotEmpty(inputdict.get('name')):
            netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object = devices.device.netflow.flow_records.flow_record.collect.ipv4.ipv4_options.ipv4_options()
            netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object.name = dev_pro_name
            netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object.ipv4_option = inputdict.get('ipv4_option')
            netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object.option = inputdict.get('option')


        #End of Device binding
        devbindobjs['netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object'] = netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object
        ...
        for dev_iterator in dev:
            ...
            if not yang.Sdk.dataExists(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect/ipv4'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name')))):
                yang.Sdk.createData(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name'))),'<ipv4/>', sdata.getSession(), False)


        netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object_payload = netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object.getxml(filter=True)
        log('netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object_payload: %s' % (netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object_payload))
        for dev_iterator in dev:
          yang.Sdk.createData(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect/ipv4'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name'))),netflow_flow_records_flow_record_collect_ipv4_ipv4_options_object_payload, sdata.getSession(), True)
    ...
    @staticmethod
    def getInstance():
        if(Ipv4Options._instance == None):
            Ipv4Options._instance = Ipv4Options()
        return Ipv4Options._instance

if __name__ == 'ipv4_options':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = Ipv4Options().getInstance()
