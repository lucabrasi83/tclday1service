...
class ArtOptions(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'art_options')
        ...
        inputdict['art_option'] = config.get_field_value('art_option')
        # END OF FETCHING THE LEAF PARAMETERS

        dev_pro_name = inputdict['name']
        if util.isNotEmpty(inputdict['art_option']):
		if inputdict['art_option'] == 'all':
			dev_pro_name = "collect_art_all"
		else:
			dev_pro_name = "collect_art_server_%s" % (inputdict['art_option'])
        ...
        if util.isNotEmpty(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name')):
          ...
          netflow_flow_records_flow_record_collect_art_object = devices.device.netflow.flow_records.flow_record.collect.art.art()
          if util.isNotEmpty(inputdict.get('name')):
            netflow_flow_records_flow_record_collect_art_art_options_object = devices.device.netflow.flow_records.flow_record.collect.art.art_options.art_options()
            netflow_flow_records_flow_record_collect_art_art_options_object.name = dev_pro_name
            netflow_flow_records_flow_record_collect_art_art_options_object.art_option = inputdict.get('art_option')


        #End of Device binding
        devbindobjs['netflow_flow_records_flow_record_collect_art_art_options_object'] = netflow_flow_records_flow_record_collect_art_art_options_object
        ...
        for dev_iterator in dev:
            ...
            if not yang.Sdk.dataExists(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect/art'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name')))):
                yang.Sdk.createData(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name'))),'<art/>', sdata.getSession(), False)


        netflow_flow_records_flow_record_collect_art_art_options_object_payload = netflow_flow_records_flow_record_collect_art_art_options_object.getxml(filter=True)
        log('netflow_flow_records_flow_record_collect_art_art_options_object_payload: %s' % (netflow_flow_records_flow_record_collect_art_art_options_object_payload))
        for dev_iterator in dev:
          yang.Sdk.createData(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect/art'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name'))),netflow_flow_records_flow_record_collect_art_art_options_object_payload, sdata.getSession(), True)
    ...
    @staticmethod
    def getInstance():
        if(ArtOptions._instance == None):
            ArtOptions._instance = ArtOptions()
        return ArtOptions._instance

if __name__ == 'art_options':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = ArtOptions().getInstance()
