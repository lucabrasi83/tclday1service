...
class TransportOptions(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        inputdict['transport_option'] = config.get_field_value('transport_option')
        ...
        if util.isNotEmpty(inputdict['transport_option']):
            if util.isNotEmpty(inputdict['tcp_options']):
                dev_pro_name = "collect_transport_%s_%s" % (inputdict['transport_option'],inputdict['tcp_options'])
            else:
                dev_pro_name = "collect_transport_%s" % (inputdict['transport_option'])
        ...
        if util.isNotEmpty(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name')):
          ...
          if util.isNotEmpty(inputdict.get('name')):
            ...
            netflow_flow_records_flow_record_collect_transport_transport_options_object.transport_option = inputdict.get('transport_option')
            netflow_flow_records_flow_record_collect_transport_transport_options_object.tcp_options = inputdict.get('tcp_options')


        #End of Device binding
        devbindobjs['netflow_flow_records_flow_record_collect_transport_transport_options_object'] = netflow_flow_records_flow_record_collect_transport_transport_options_object
        ...
        for dev_iterator in dev:
            ...
            if not yang.Sdk.dataExists(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect/transport'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name')))):
                yang.Sdk.createData(dev_iterator.url+'/l3features:netflow/flow-records/flow-record=%s/collect'%(util.make_interfacename(inputdict.get('day1services_day1service_netflow_netflow_flow_records_flow_record_name'))),'<transport/>', sdata.getSession(), False)


        netflow_flow_records_flow_record_collect_transport_transport_options_object_payload = netflow_flow_records_flow_record_collect_transport_transport_options_object.getxml(filter=True)
        log('netflow_flow_records_flow_record_collect_transport_transport_options_object_payload: %s' % (netflow_flow_records_flow_record_collect_transport_transport_options_object_payload))
        for dev_iterator in dev:
    ...
    @staticmethod
    def getInstance():
        if(TransportOptions._instance == None):
            TransportOptions._instance = TransportOptions()
        return TransportOptions._instance

if __name__ == 'transport_options':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = TransportOptions().getInstance()
