...
class FlowExports(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        netflow_flow_exports_object.legacy_netflow = inputdict.get('legacy_netflow')
        netflow_flow_exports_object.source = inputdict.get('source')
        netflow_flow_exports_object.interface_names = inputdict.get('interface_names')
        netflow_flow_exports_object.version = inputdict.get('version')


        #End of Device binding
        devbindobjs['netflow_flow_exports_object'] = netflow_flow_exports_object
        ...
        netflow_flow_exports_object_payload = netflow_flow_exports_object.getxml(filter=True)
        log('netflow_flow_exports_object_payload: %s' % (netflow_flow_exports_object_payload))
        for dev_iterator in dev:
          yang.Sdk.createData(dev_iterator.url+'/l3features:netflow',netflow_flow_exports_object_payload, sdata.getSession(), True)
    ...
    @staticmethod
    def getInstance():
        if(FlowExports._instance == None):
            FlowExports._instance = FlowExports()
        return FlowExports._instance

if __name__ == 'flow_exports':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = FlowExports().getInstance()
