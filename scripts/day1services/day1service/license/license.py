...
from servicemodel.controller.devices.device import license
import service_customization


class License(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'license')
        ...
        inputdict['license_token'] = config.get_field_value('license_token')
        inputdict['dest_address'] = config.get_field_value('dest_address')
        ...
        #license_object = devices.device.license.license()
        license_object = license.license()
        license_object.license_token = inputdict.get('license_token')
        license_object.dest_address = inputdict.get('dest_address')
        license_object.source_interface = inputdict.get('source_interface')


        #End of Device binding
        devbindobjs['license_object'] = license_object
        ...
        license_object_payload = license_object.getxml(filter=True)
        log('license_object_payload: %s' % (license_object_payload))
        for dev_iterator in dev:
          yang.Sdk.createData(dev_iterator.url+'',license_object_payload, sdata.getSession(), True)
    ...
    @staticmethod
    def getInstance():
        if(License._instance == None):
            License._instance = License()
