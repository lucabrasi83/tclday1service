...
from servicemodel.controller.devices.device import call_home
import service_customization


class CallHome(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'call_home')
        ...
        inputdict['profile'] = config.get_field_value('profile')
        inputdict['destination'] = config.get_field_value('destination')
        inputdict['no_destination'] = config.get_field_value('no_destination')
        ...
        #call_home_object = devices.device.call_home.call_home()
        call_home_object = call_home.call_home()
        call_home_object.profile = inputdict.get('profile')
        call_home_object.destination = inputdict.get('destination')
        call_home_object.no_destination = inputdict.get('no_destination')


        #End of Device binding
        devbindobjs['call_home_object'] = call_home_object
        ...
        call_home_object_payload = call_home_object.getxml(filter=True)
        log('call_home_object_payload: %s' % (call_home_object_payload))
        for dev_iterator in dev:
          yang.Sdk.createData(dev_iterator.url+'',call_home_object_payload, sdata.getSession(), True)
    ...
    @staticmethod
    def getInstance():
        if(CallHome._instance == None):
            CallHome._instance = CallHome()
        return CallHome._instance

    def rollbackCreate(self, id, sdata):
        log('rollback: id = %s, sdata = %s' % (id, sdata))
        self.delete(id,sdata)

if __name__ == 'call_home':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = CallHome().getInstance()
