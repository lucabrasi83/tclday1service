...
Names of Leafs for this Yang Entity
         name-server            maps-to  /ac:devices/ac:device/dns:dns-server/name-server/server
            vrf-name            maps-to  /ac:devices/ac:device/dns:dns-server/name-server/vrf-name
"""
...
class ServiceDataCustomization:
    ...
    def process_service_device_bindings(smodelctx, sdata, dev, **kwargs):
      ...
      if modify:
        ...
        tclday0config.tclday0config_grouping_lib.dns_def_customization.grouping_create_dns_def_name_server(smodelctx, sdata, dev, **kwargs)


    @staticmethod
    def process_service_update_data(smodelctx, sdata, **kwargs):
      """callback called for update operation"""
      #raise Exception('Update forbidden  for node name-server at path day1services/day1service/dns/name-server')
      modify = False
      if modify and kwargs is not None:
        for key, value in kwargs.iteritems():
          log("%s == %s" %(key,value))
      ...
      if modify:
        ...
        inputdict = kwargs['inputdict']
        import tclday0config.tclday0config_grouping_lib.dns_def_customization
        tclday0config.tclday0config_grouping_lib.dns_def_customization.grouping_update_dns_def_name_server(smodelctx, sdata, **kwargs)
