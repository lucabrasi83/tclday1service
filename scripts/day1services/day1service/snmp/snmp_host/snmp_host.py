...
class SnmpHost(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'snmp_host')
        ...
        inputdict['snmp_community'] = config.get_field_value('snmp_community')
        ...
        if util.isNotEmpty(inputdict.get('host')):
          #snmp_snmp_server_object = devices.device.snmp.snmp_server.snmp_server()
          snmp_snmp_server_object = snmp.snmp_server.snmp_server()
          snmp_snmp_server_object.snmp_server_ip = inputdict.get('host')
          snmp_snmp_server_object.context_name = inputdict.get('vrf_name')
          snmp_snmp_server_object.snmp_version = inputdict.get('version')
          snmp_snmp_server_object.community = inputdict.get('snmp_community')


        #End of Device binding
        devbindobjs['snmp_snmp_server_object'] = snmp_snmp_server_object
        ...
        snmp_snmp_server_object_payload = snmp_snmp_server_object.getxml(filter=True)
        log('snmp_snmp_server_object_payload: %s' % (snmp_snmp_server_object_payload))
        for dev_iterator in dev:
          yang.Sdk.createData(dev_iterator.url+'/basicDeviceConfigs:snmp',snmp_snmp_server_object_payload, sdata.getSession(), True)
    ...
    @staticmethod
    def getInstance():
        if(SnmpHost._instance == None):
            SnmpHost._instance = SnmpHost()
        return SnmpHost._instance

    def rollbackCreate(self, id, sdata):
        log('rollback: id = %s, sdata = %s' % (id, sdata))
        self.delete(id,sdata)

if __name__ == 'snmp_host':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = SnmpHost().getInstance()
