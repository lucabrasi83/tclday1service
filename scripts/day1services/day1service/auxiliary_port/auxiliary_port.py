...
from servicemodel.controller.devices.device import auxiliary_port 
import service_customization


class AuxiliaryPort(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'auxiliary_port')
        ...
        inputdict['auxiliary_port_line'] = config.get_field_value('auxiliary_port_line')
        inputdict['no_exec'] = config.get_field_value('no_exec')
        if inputdict.get('no_exec') is None:
          inputdict['no_exec'] = 'False'
        ...
        #auxiliary_port_object = devices.device.auxiliary_port.auxiliary_port()
        auxiliary_port_object = auxiliary_port.auxiliary_port()
        auxiliary_port_object.auxiliary_port_line = inputdict.get('auxiliary_port_line')
        auxiliary_port_object.no_exec = inputdict.get('no_exec')


        #End of Device binding
        devbindobjs['auxiliary_port_object'] = auxiliary_port_object
        ...
        auxiliary_port_object_payload = auxiliary_port_object.getxml(filter=True)
        log('auxiliary_port_object_payload: %s' % (auxiliary_port_object_payload))
        for dev_iterator in dev:
          yang.Sdk.createData(dev_iterator.url+'',auxiliary_port_object_payload, sdata.getSession(), True)
    ...
    @staticmethod
    def getInstance():
        if(AuxiliaryPort._instance == None):
            AuxiliaryPort._instance = AuxiliaryPort()
        return AuxiliaryPort._instance

    def rollbackCreate(self, id, sdata):
        log('rollback: id = %s, sdata = %s' % (id, sdata))
        self.delete(id,sdata)

if __name__ == 'auxiliary_port':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = AuxiliaryPort().getInstance()
